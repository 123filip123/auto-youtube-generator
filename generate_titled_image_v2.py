...
def generate_titled_images_for_items_v2(items_json: str):
    """
    Generate titled images for each item in the JSON list.
    Handles both full description images and sentence-specific images from v2 generator.
    ...
    for i, item in enumerate(items, start=1):
        try:
            # First, generate titled image for the full description image
            full_image_path = f"outputs/image_output/item_{i:02d}_full.png"
            full_output_filename = f"item_{i:02d}_full"
            full_image_path = generate_titled_image(
                full_image_path,
                item['title'],
                full_output_filename
            )
            image_paths.append(full_image_path)
            print(f"Generated titled image for full description: {full_image_path}")
            
            # Then, generate titled images for each sentence
            sentences = item['description'].split('. ')
            for j, sentence in enumerate(sentences, start=1):
                if not sentence.strip():  # Skip empty sentences
                    continue
                    
                # Get the source image path for this sentence
                sentence_image_path = f"outputs/image_output/item_{i:02d}_sentence_{j:02d}.png"
                
                # Generate the titled image
                sentence_output_filename = f"item_{i:02d}_sentence_{j:02d}"
                sentence_image_path = generate_titled_image(
                    sentence_image_path,
                    item['title'],
                    sentence_output_filename
                )
                image_paths.append(sentence_image_path)
                print(f"Generated titled image for sentence {j}: {sentence_image_path}")
            
        except Exception as e:
            print(f"Failed to generate titled images for item {i}: {str(e)}")
            continue
